...
retry:;
    ...
    /* The dict is locked while its items are copied out, so a concurrent
       mutation cannot desynchronize the loop from the buffer sizes; the
       loop body performs no allocation, so the critical section is not
       suspended mid-loop. */
    ...
    if (i != nkwargs) {
        /* The dict was resized between sizing the buffers and acquiring
           its lock; redo the allocations with the new size. */
        assert(i == 0);
        PyMem_Free(stack - 1);
        Py_DECREF(kwnames);
