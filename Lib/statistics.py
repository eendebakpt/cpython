...
def _sum(data):
    ...
    for typ, values in groupby(data, type):
        types.add(typ)
        for n, d in _exact_ratios(typ, values):
            count += 1
...
def _exact_ratios(typ, values):
    """Yield exact (numerator, denominator) pairs for *values*, all of type *typ*.

    Equivalent to map(_exact_ratio, values), but resolves the conversion
    once per group instead of dispatching on every individual value.
    """
    try:
        as_integer_ratio = typ.as_integer_ratio
    except AttributeError:
        # Rare types (e.g. an Integral ABC without as_integer_ratio).
        yield from map(_exact_ratio, values)
        return
    for x in values:
        try:
            yield as_integer_ratio(x)
        except (OverflowError, ValueError):
            yield _exact_ratio(x)       # x is a float or Decimal NAN or INF
